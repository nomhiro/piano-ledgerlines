"""metrics.md の指標を実装し、参照譜と演奏から小節別スコアを算出する。

articulation は M4 の検証で offset が信頼できないことが判明したため実装しない
（analyze_offsets.py / analyze_legato.py を参照）。

BeatMap は matched 音符の (拍, 秒) 対から推定する。
テンポの揺れは BeatMap が吸収し、rhythm は「拍に直したうえでの残差」だけを見る。
これにより rhythm と tempo が分離される。
"""
...
def compute(
    ...
    # dynamics のアフィン正規化は曲全体で行うので、先に小節平均を集める
    ...
    if len(actual) >= 2 and np.std(actual) > 1e-6:
        ...
        # スケールを制限しないと、強弱を潰した演奏まで完全にフィットしてしまう
    ...
    for m, notes in sorted(ref_by_measure.items()):
        ...
        # rhythm: 拍に逆変換した残差。小節全体のオフセットは除去する
        deltas = []
        for n in notes:
...
def load_est(path: Path) -> tuple[list[dict], list[tuple[float, float]]]:
...
def main() -> int:
    ...
    for align_path in sorted(args.alignment.glob("*.alignment.json")):
        ...
        ref_pedal = pedal_intervals(pretty_midi.PrettyMIDI(str(args.dataset / f"{name}.ref.mid")))
        result = compute(reference, est_notes, alignment, est_pedal, ref_pedal)
