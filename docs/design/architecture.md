...
> **M4 を受けた再評価**: 解析そのものが 3分の曲で **4.3分**かかることが判明したため、
> コールドスタートの 20-40秒は**相対的な重要度が下がった**（全体の 10% 未満）。
...
>
> 高速化が必要になった場合の選択肢は、ONNX Runtime への移行、int8 量子化、
> または GPU（NC 系）への移行である。いずれも M5 以降で検証する。
...
| 解析時間 | **平均 260秒**（4 vCPU / 8 GiB、M4 実測 RTF 1.25 に基づく） |
| LLM トークン | 2,900 / テイク |

> **M4 で解析時間の前提が 55秒 → 260秒 に修正された（4.7倍）。**
> 当初は採譜が実時間の 1/3 程度で終わると見積もっていたが、
> 実測では実時間の 1.25 倍かかる。**この差はコスト試算を直撃する。**

### 7.2 内訳（月額、円。1 USD = 155円換算の概算）

| # | サービス | 算出根拠 | 月額 |
|---|---|---|---|
| 1 | Container Apps（Worker） | 20,000件 × 260秒 = 1,444 vCPU時間 × 4vCPU/8GiB相当<br/>+ 日中 minReplicas:1 の待機分 | **104,000** |
...
> **目標の10万円を大きく超過する。** M4 以前の試算（85,500円）は
> 採譜速度の楽観的な前提に依存していた。
> コストの **64% が採譜の CPU 推論**に集中しており、ここを下げないと成立しない。
...
| **ONNX Runtime + int8 量子化** | -30〜50% | M5 で検証。精度低下がないことの確認が必要 |
| 無音区間のスキップ | -5〜10% | 録音の前後の空白を切り落とす |

上3つが効けば worker は 104,000 → **25,000〜35,000 円**に収まり、合計は 9万円前後となる。
ただし**いずれも未検証**であり、M5 の早い段階で確かめる必要がある。
...
#### ② Analysis Worker（104,000円 / 64%）

**ここが最大のコスト項目**。7.2.1 の施策を優先度順に実施する。

| 施策 | 削減見込み | 状態 |
|---|---|---|
| ONNX Runtime 化 + int8 量子化による推論高速化 | -30〜50% | **M5 で最優先検証** |
...
| # | 論点 | 状態 |
|---|---|---|
| Q1 | 採譜を CPU で実行できるか（GPU 要否） | **可（M4 で実証）**。4 vCPU で RTF 1.25。ただし解析は 3分の曲で 4.3分かかるため**非同期前提**。コストは 7.2 のとおり要削減 |
...
| Q6 | **ONNX / int8 量子化で採譜を高速化できるか** | **新規（M4 で判明）**。コストの 64% を占めるため M5 最優先。精度低下がないことの確認が必要 |
| Q7 | **弾き直し・途中停止を含む演奏をアライメントできるか** | **新規（M4 で判明）**。DTW は単調性を仮定するため原理的に非対応。初中級者には頻出。M5 で部分マッチの連結を設計する |
