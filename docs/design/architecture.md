...
| Analysis Worker | **4** | **8 GiB** | 採譜モデルの推論。M4 で実測して調整 |
...
| # | サービス | 算出根拠 | 月額 |
|---|---|---|---|
| 1 | Container Apps（Worker） | 20,000件 × 55秒 = 306 vCPU時間 × 4vCPU相当<br/>+ 日中 minReplicas:1 の待機分 | **28,000** |
...
#### ② Analysis Worker（28,000円 / 33%）

| 施策 | 削減見込み |
|---|---|
| ONNX Runtime 化による推論高速化 | -30% |
| 音声の 16kHz モノラル化（既に織り込み済み） | — |
| 深夜の minReplicas を 0 に | 既に織り込み済み |
| チャンク分割の並列化（レイテンシは改善、コストは中立） | 0% |

### 7.4 スケール時の見通し

| MAU | テイク/月 | 月額概算 | 1ユーザーあたり |
|---|---|---|---|
| 1,000 | 20,000 | 85,000 | 85円 |
| 10,000 | 200,000 | 620,000 | 62円 |
| 50,000 | 1,000,000 | 2,700,000 | 54円 |

固定費（Front Door、常時レプリカ）が薄まるため、単価は逓減する。

**課金設計への示唆**: 変動費が1ユーザーあたり月50-85円かかるため、
完全無料のフリーミアムは成立しにくい。**無料枠は月5テイクまで**といった制限が必要。
...
| # | 論点 | 決定時期 |
|---|---|---|
| Q1 | 採譜を CPU で実行できるか（GPU 要否） | **M4。アーキテクチャとコストの前提が変わる最重要項目** |
| Q2 | 講評に使うモデルのグレード | M4 の品質評価後 |
| Q3 | フリーミアムの無料枠（月何テイクまで） | ビジネス判断。7.4 の単価を根拠に |
