...
**スレッド数を3倍にしても8%しか改善しない**（メモリ帯域律速）。
3分の曲で採譜に 3.75 分、パイプライン全体で約 4.3 分かかる。

| 課題 | 対策 | 状態 |
|---|---|---|
| **推論が遅い（コストの64%）** | ONNX Runtime + int8 量子化 | **M5 最優先。-30〜50% を見込む** |
| vCPU を増やしても速くならない | 4 vCPU で据え置き。レプリカ数で並列度を稼ぐ | 確定 |
| 長時間音源のメモリ | ライブラリが内部で分割推論する（`segment_samples`） | 確認済み |
| 初回のモデルロードが遅い | コンテナイメージにモデル（172MB）を同梱 | 確定 |
...
3. 非対角遷移に STEP_PENALTY = 0.05 を課した DTW でパスを求める
```
...
> **弾き直し・途中停止に原理的に対応できない。**
> DTW は対応の**単調性**を仮定するため、「間違えて3小節戻ってやり直す」演奏では
> 戻った部分以降のアライメントが崩れる。
>
> これは**初中級者の練習録音では頻出**であり、本プロダクトの主要ターゲットに直撃する。
> M5 で以下を設計・検証する。
>
> | 案 | 内容 |
> |---|---|
> | A | 部分マッチの連結。演奏を区間に分割し、それぞれを参照譜の部分列にマッチさせ、非単調な連結を許す |
> | B | 繰り返し検出の前処理。演奏内で自己相似な区間を見つけて重複を除去してから通常のアライメントを掛ける |
> | C | UI で「弾き直しました」と申告させる（暫定回避策） |
>
> M4 の検証データ（MAESTRO のコンクール録音）には弾き直しが含まれていないため、
> **この課題は未計測**である。M5 では実際の練習録音を集めて評価する必要がある。
