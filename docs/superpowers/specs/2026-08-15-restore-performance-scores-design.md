...
- `NOISE_VELOCITY_RATIO = 0.50` を 0.90 まで緩めても `noiseShare` の上限は約 0.20 で、
  Task 9 の決定規則が要求する 0.50 には届かない。**この上限（規則2@0.90 の43件 + 規則4@0.90 の
  6件 = 51件から算出）は規則2 と規則4 の重複排除をしていないため過大評価であり、真の上限は
  0.20 以下である**（結論「0.50 に届かない」は安全側に変わらない）
...
3. **`noiseShare` 上限 0.20 の計算は規則2 と規則4 の重複を排除していない**（上記のとおり）ため、
   真の上限はさらに下がる可能性がある。フェーズ2 で閾値を確定する際は重複排除した集計を使うこと
