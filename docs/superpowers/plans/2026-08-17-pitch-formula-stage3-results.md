...
（`51` が単純和である点について: 規則3・規則2@0.90・規則4@0.90 に該当する extra の集合を
`poc/scripts/analyze_real_take_extra.py` の `overlapAt090` で実測したところ、この録音では
3集合は互いに素（`pairwiseIntersections` すべて `[]`）で、単純和 51 は重複のない union と
一致した。録音1件の実測であり、一般に重複が無いとは限らない。）
