...
|---|---|---|---|
| 1 duplicate | matched と同音高が ±50ms 内: **0** | 0 | 採譜器が同音高の二重検出を出していない |
| 2 harmonic | ±50ms 内にオクターブ違いの matched: **69** | 0 | **69 件すべてが velocity 比 0.50 を満たさない** |
| 3 spurious | `duration < 60ms`: 65 / `velocity < 40`: 15 | **2** | **AND なので同時に満たすのが 2 件だけ** |
| 4 reverb | ペダル区間は 158 あり | 0 | 同音高 + velocity 比 0.50 の組が無い |

- **規則1 が構造的に空振りしている。** 225 件（258 件の 87%）は ±50ms 内に matched 音符を
  持つのに、同音高のものは1件も無い。`piano_transcription_inference` は同じ音高について
  note-off を経ずに次の note-on を出さないため、この採譜器では「同音高の二重検出」が
  そもそも発生しないと考えられる。規則1 は**この採譜器に対しては死んでいる**。
  （`retake` として `extra` から除かれた 128 件についても、規則1 に該当するものは 0 件だった。
  「二重検出が retake に吸収されて隠れている」わけではない。）
- **`NOISE_VELOCITY_RATIO = 0.50` が実測分布に対して厳しすぎる。** 規則2 の前段には 69 件が
  該当するのに、velocity が元の半分未満のものは 0 件。extra の velocity は
  p10/p50/p90 = 44/60/82（最小 21）で、採譜器の velocity 推定はこの範囲に圧縮されている。
  規則2 と規則4 はどちらもこの比を必須条件に持つため、両方とも 0 件になる。
- **規則3 の AND が効きすぎている。** 片方だけなら 65 件 / 15 件が該当する。

いずれも**閾値が暫定値である**ことの帰結で（`align.py:302-307`、設計 §4.2 / §9.3）、実測根拠を
まだ持たない値が実データに対して機能していないことが判明したという位置づけである。
**閾値の確定はフェーズ2 の仕事であり、このタスクでは変更していない。**
